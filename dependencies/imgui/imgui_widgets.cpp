...
void ui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    if (*selected != num) {
        if (hovered) {
            window->DrawList->AddRectFilledMultiColor(pos, pos + size, 
                ImColor(32 / 255.f, 32 / 255.f, 32 / 255.f, g.Style.Alpha), 
                ImColor(32 / 255.f, 32 / 255.f, 32 / 255.f, g.Style.Alpha), 
                ImColor(20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha), 
                ImColor(20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha));
        }
        else {
            window->DrawList->AddRectFilledMultiColor(pos, pos + size, 
                ImColor(28 / 255.f, 28 / 255.f, 28 / 255.f, g.Style.Alpha), 
                ImColor(28 / 255.f, 28 / 255.f, 28 / 255.f, g.Style.Alpha), 
                ImColor(18 / 255.f, 18 / 255.f, 18 / 255.f, g.Style.Alpha), 
                ImColor(18 / 255.f, 18 / 255.f, 18 / 255.f, g.Style.Alpha));
        }
    }
    else {
        const auto& menu_color = settings::m_menu_color;
        const auto accent_r = menu_color[0];
        const auto accent_g = menu_color[1];
        const auto accent_b = menu_color[2];
        
        window->DrawList->AddRectFilledMultiColor(pos, pos + size, 
            ImColor(accent_r * 0.15f, accent_g * 0.15f, accent_b * 0.15f, g.Style.Alpha), 
            ImColor(accent_r * 0.15f, accent_g * 0.15f, accent_b * 0.15f, g.Style.Alpha), 
            ImColor(accent_r * 0.08f, accent_g * 0.08f, accent_b * 0.08f, g.Style.Alpha), 
            ImColor(accent_r * 0.08f, accent_g * 0.08f, accent_b * 0.08f, g.Style.Alpha));
        
        window->DrawList->AddLine(ImVec2(pos.x, pos.y + size.y - 2), ImVec2(pos.x + size.x, pos.y + size.y - 2), 
            ImColor(accent_r, accent_g, accent_b, g.Style.Alpha), 2.f);
    }

    window->DrawList->AddRect(pos - ImVec2(1, 1), pos + size + ImVec2(1, 1), ImColor(5 / 255.f, 5 / 255.f, 5 / 255.f, g.Style.Alpha), 0.f, 0, 1.f);
    window->DrawList->AddRect(pos, pos + size, ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha), 0.f, 0, 1.f);

    if (pressed)
        *selected = num;

    ImColor textColor = ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha);
    if (hovered && *selected != num)
        textColor = ImColor(200 / 255.f, 200 / 255.f, 200 / 255.f, g.Style.Alpha);
    if (*selected == num) {
        const auto& menu_color = settings::m_menu_color;
        textColor = ImColor(menu_color[0], menu_color[1], menu_color[2], g.Style.Alpha);
    }

    window->DrawList->AddText(ImVec2(pos.x + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1), textColor, label);
}

void ui::TabButtonAimware(const char* label, int* selected, int num, int total, const ImColor& accent_color) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const float tab_spacing = 2.f;
    const float total_tab_width = window->Size.x - 30.f - (tab_spacing * (total - 1));
    const float tab_width = total_tab_width / total;
    
    ImVec2 pos = window->Pos + ImVec2(15, 50) + ImVec2((tab_width + tab_spacing) * num, 0);
    ImVec2 size = ImVec2(tab_width, 30);
    ...
    const bool is_active = (*selected == num);

    if (is_active) {
        window->DrawList->AddRectFilled(
            pos, pos + size,
            ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 35)),
            4.f
        );
        window->DrawList->AddRectFilledMultiColor(
            pos, pos + size,
            ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 20)),
            ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 10)),
            ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 10)),
            ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 20))
        );
    } else {
        if (hovered) {
            window->DrawList->AddRectFilled(
                pos, pos + size,
                ImColor(28, 28, 28, static_cast<int>(g.Style.Alpha * 255)),
                4.f
            );
            window->DrawList->AddRectFilledMultiColor(
                pos, pos + size,
                ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 8)),
                ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 4)),
                ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 4)),
                ImColor(accent_color.r, accent_color.g, accent_color.b, static_cast<int>(g.Style.Alpha * 8))
            );
        } else {
            window->DrawList->AddRectFilled(
                pos, pos + size,
                ImColor(20, 20, 20, static_cast<int>(g.Style.Alpha * 255)),
                4.f
            );
        }
    }
    ...
    window->DrawList->AddRect(
        pos, pos + size,
        is_active ? accent_color : ImColor(40, 40, 40, static_cast<int>(g.Style.Alpha * 255)),
        4.f, 0, is_active ? 1.5f : 1.f
    );
    ...
    ImColor textColor = ImColor(140, 140, 140, static_cast<int>(g.Style.Alpha * 255));
    if (hovered && !is_active)
        textColor = ImColor(200, 200, 200, static_cast<int>(g.Style.Alpha * 255));
    if (is_active)
        textColor = accent_color;

    window->DrawList->AddText(
        ImVec2(pos.x + size.x / 2 - label_size.x / 2, pos.y + size.y / 2 - label_size.y / 2 - 1),
        textColor, label
    );
}
...
bool ui::Checkbox(const char* label, bool* v, bool safe)
{
    ...
    const ImRect checkbox_bb(pos, pos + ImVec2(checkbox_size, checkbox_size));
    const ImRect total_bb(pos, pos + ImVec2(checkbox_size + spacing + label_size.x, ImMax(checkbox_size, label_size.y)));
    ...
    const ImVec2 checkbox_min = checkbox_bb.Min;
    const ImVec2 checkbox_max = checkbox_bb.Max;
    
    const auto& menu_color = settings::m_menu_color;
    const ImColor accent_color(menu_color[0], menu_color[1], menu_color[2], g.Style.Alpha);
    
    if (*v)
    {
        ImColor bg_color = hovered ? ImColor(accent_color.r * 0.9f, accent_color.g * 0.9f, accent_color.b * 0.9f, g.Style.Alpha) : accent_color;
        window->DrawList->AddRectFilled(checkbox_min, checkbox_max, bg_color, rounding);
        window->DrawList->AddRect(checkbox_min, checkbox_max, ImColor(accent_color.r * 0.5f, accent_color.g * 0.5f, accent_color.b * 0.5f, g.Style.Alpha), rounding, 0, 1.5f);
        
        const float checkmark_thickness = 2.0f;
        const ImVec2 center = ImVec2((checkbox_min.x + checkbox_max.x) * 0.5f, (checkbox_min.y + checkbox_max.y) * 0.5f);
        const float sz = checkbox_size * 0.4f;
        const ImVec2 p1 = ImVec2(center.x - sz * 0.5f, center.y);
        const ImVec2 p2 = ImVec2(center.x - sz * 0.2f, center.y + sz * 0.4f);
        const ImVec2 p3 = ImVec2(center.x + sz * 0.5f, center.y - sz * 0.4f);
        window->DrawList->AddLine(p1, p2, ImColor(255, 255, 255, static_cast<int>(g.Style.Alpha * 255)), checkmark_thickness);
        window->DrawList->AddLine(p2, p3, ImColor(255, 255, 255, static_cast<int>(g.Style.Alpha * 255)), checkmark_thickness);
    }
    else
    {
        ImColor bg_color;
        if (hovered)
            bg_color = ImColor(45 / 255.f, 45 / 255.f, 45 / 255.f, g.Style.Alpha);
        else
            bg_color = ImColor(35 / 255.f, 35 / 255.f, 35 / 255.f, g.Style.Alpha);
        
        window->DrawList->AddRectFilled(checkbox_min, checkbox_max, bg_color, rounding);
        window->DrawList->AddRect(checkbox_min, checkbox_max, ImColor(60 / 255.f, 60 / 255.f, 60 / 255.f, g.Style.Alpha), rounding, 0, 1.0f);
    }

    ImColor text_color = safe ? ImColor(220 / 255.f, 220 / 255.f, 220 / 255.f, g.Style.Alpha) : ImColor(220 / 255.f, 220 / 255.f, 100 / 255.f, g.Style.Alpha);
    if (hovered)
        text_color = ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, g.Style.Alpha);
    
    window->DrawList->AddText(ImVec2(checkbox_max.x + spacing, checkbox_min.y + (checkbox_size - label_size.y) * 0.5f), text_color, label);

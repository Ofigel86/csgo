			...
			if (shot.m_target.m_entry
				...
				if (shot.m_server_info.m_hurt_tick) {
					if (shot.m_target.m_lag_record->m_trying_to_resolve) {
						...
						shot.m_target.m_entry->register_resolve_hit(new_anim_side);
					}
					else {
						shot.m_target.m_entry->register_resolve_hit(shot.m_target.m_lag_record->m_side);
					}
				}
				else {
					...
					if (!trace.m_hit_entity
						...
						if ((shot.m_target.m_entry->m_player->flags() & valve::e_ent_flags::fake_client)
							...
							if (shot.m_target.m_lag_record->m_trying_to_resolve
								...
								shot.m_target.m_entry->register_resolve_miss(shot.m_target.m_lag_record->m_side);

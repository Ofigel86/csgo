...
namespace supremacy::hacks {
	...
	void c_anim_sync::find_server_foot_yaw(
		...
		current->m_side = std::clamp(entry.m_prev_side, 0, 4);
		current->m_type = 1;

		if (!previous) {
			current->m_type = 5;
			entry.m_prev_side = current->m_side = 0;
			entry.m_prev_type = 0;
			goto end;
		}
		...
		std::array<float, 5u> scores{};
		std::array<int, 5u> reasons{};
		std::array<float, 5u> reason_strength{};

		auto add_score = [&](int side, float value, int reason_code) {
			if (side < 0 || side >= static_cast<int>(scores.size()) || !std::isfinite(value))
				return;

			scores[side] += value;
			if (reason_code && value > reason_strength[side]) {
				reason_strength[side] = value;
				reasons[side] = reason_code;
			}
		};

		for (int side = 0; side < 5; ++side) {
			const auto total_attempts = entry.m_side_hits[side] + entry.m_side_misses[side];
			const auto hit_ratio = total_attempts > 0 
				? static_cast<float>(entry.m_side_hits[side]) / static_cast<float>(total_attempts)
				: 0.f;
			
			if (entry.m_side_hits[side] > 0) {
				const auto hit_bonus = static_cast<float>(entry.m_side_hits[side]) * (7.f + hit_ratio * 5.f);
				const auto consistency_bonus = hit_ratio > 0.7f ? entry.m_side_hits[side] * 2.f : 0.f;
				add_score(side, hit_bonus + consistency_bonus, 4);
			}

			if (entry.m_side_misses[side] > 0) {
				const auto miss_penalty = static_cast<float>(-entry.m_side_misses[side]) * (7.f + (1.f - hit_ratio) * 3.f);
				const auto inconsistency_penalty = hit_ratio < 0.3f && total_attempts > 2 ? entry.m_side_misses[side] * 1.5f : 0.f;
				add_score(side, miss_penalty - inconsistency_penalty, 4);
			}
		}

		add_score(current->m_side, 3.f, reasons[current->m_side]);

		const int resolver_mode = sdk::g_config_system->resolver_mode;
		
		if (entry.m_trace_side && entry.m_try_trace_resolver) {
			const auto trace_confidence = entry.m_trace_side >= 1 && entry.m_trace_side <= 2 ? 1.f : 0.8f;
			if (resolver_mode == 0) {
				add_score(entry.m_trace_side, 55.f * trace_confidence, 2);
			} else if (resolver_mode == 1) {
				add_score(entry.m_trace_side, 75.f * trace_confidence, 2);
			} else if (resolver_mode == 2) {
				add_score(entry.m_trace_side, 65.f * trace_confidence, 2);
			}
		}

		const auto speed_2d = current->m_velocity.length_2d();
		const bool stationary = speed_2d <= 0.1f;

		if (stationary) {
			if (entry.m_try_lby_resolver) {
				const auto angle_diff = math::angle_diff(current->m_eye_angles.y, current->m_sides.at(0u).m_foot_yaw);
				const auto abs_angle_diff = std::fabs(angle_diff);
				
				if (resolver_mode == 0) {
					if (abs_angle_diff > 28.f) {
						const auto lby_side = angle_diff < 0.f ? 2 : 1;
						const auto lby_score = 65.f + abs_angle_diff * 0.7f;
						const auto confidence_boost = abs_angle_diff > 50.f ? 10.f : 0.f;
						add_score(lby_side, lby_score + confidence_boost, 1);
					}
				} else if (resolver_mode == 1) {
					if (abs_angle_diff > 23.f) {
						const auto lby_side = angle_diff < 0.f ? 2 : 1;
						const auto lby_score = 80.f + abs_angle_diff * 0.7f;
						const auto confidence_boost = abs_angle_diff > 50.f ? 12.f : 0.f;
						add_score(lby_side, lby_score + confidence_boost, 1);
						
						const auto opposite_side = angle_diff < 0.f ? 1 : 2;
						if (abs_angle_diff > 48.f) {
							add_score(opposite_side, -25.f, 1);
						}
					}
				} else if (resolver_mode == 2) {
					if (abs_angle_diff > 26.f) {
						const auto lby_side = angle_diff < 0.f ? 2 : 1;
						const auto lby_score = 70.f + abs_angle_diff * 0.7f;
						const auto confidence_boost = abs_angle_diff > 50.f ? 11.f : 0.f;
						add_score(lby_side, lby_score + confidence_boost, 1);
					}
				}
			}
			...
			if (entry.m_try_anim_resolver) {
				std::array<float, 5u> deltas{};
				std::array<bool, 5u> valid_rates{};

				auto eval_rate = [&](int side, float rate, bool low) {
					if (!std::isfinite(rate) || rate <= 0.f) {
						valid_rates[side] = false;
						return;
					}

					valid_rates[side] = true;
					const auto delta = std::fabs(current->m_server_rate - rate);
					deltas[side] = delta;

					if (resolver_mode == 0) {
						const auto weight = low ? 48.f : 58.f;
						const auto base_value = (1.f / (delta + 0.0003f)) * weight;
						const auto penalty = delta * (low ? 4.8f : 6.8f);
						const auto value = base_value - penalty;
						
						if (delta < 0.04f) {
							add_score(side, value + 18.f, 3);
						} else if (delta < 0.08f) {
							add_score(side, value + 10.f, 3);
						} else {
							add_score(side, value, 3);
						}
					} else if (resolver_mode == 1) {
						const auto weight = low ? 53.f : 63.f;
						const auto base_value = (1.f / (delta + 0.0003f)) * weight;
						const auto penalty = delta * (low ? 5.2f : 7.2f);
						const auto value = base_value - penalty;
						
						if (delta < 0.04f) {
							add_score(side, value + 25.f, 3);
						} else if (delta < 0.08f) {
							add_score(side, value + 12.f, 3);
						} else {
							add_score(side, value, 3);
						}
					} else if (resolver_mode == 2) {
						const auto weight = low ? 50.f : 60.f;
						const auto base_value = (1.f / (delta + 0.0003f)) * weight;
						const auto penalty = delta * (low ? 5.f : 7.f);
						const auto value = base_value - penalty;
						
						if (delta < 0.04f) {
							add_score(side, value + 20.f, 3);
						} else if (delta < 0.08f) {
							add_score(side, value + 11.f, 3);
						} else {
							add_score(side, value, 3);
						}
					}
				...
				eval_rate(0, current->m_zero_rate, false);
				eval_rate(1, current->m_negative_rate, false);
				eval_rate(2, current->m_positive_rate, false);
				eval_rate(3, current->m_low_negative_rate, true);
				eval_rate(4, current->m_low_positive_rate, true);

				const auto min_normal_delta = std::min({
					valid_rates[0] ? deltas[0] : std::numeric_limits<float>::max(),
					valid_rates[1] ? deltas[1] : std::numeric_limits<float>::max(),
					valid_rates[2] ? deltas[2] : std::numeric_limits<float>::max()
				});

				const auto min_low_delta = std::min({
					valid_rates[3] ? deltas[3] : std::numeric_limits<float>::max(),
					valid_rates[4] ? deltas[4] : std::numeric_limits<float>::max()
				});

				if (std::isfinite(min_low_delta) && min_low_delta < min_normal_delta * 0.65f) {
					if (valid_rates[3] && deltas[3] < deltas[4]) {
						add_score(3, 28.f, 3);
						if (valid_rates[4])
							add_score(4, -10.f, 3);
					}
					else if (valid_rates[4] && deltas[4] < deltas[3]) {
						add_score(4, 28.f, 3);
						if (valid_rates[3])
							add_score(3, -10.f, 3);
					}
				...
				if (current->m_sideways) {
					if (valid_rates[3] && valid_rates[4]) {
						const auto diff = std::fabs(deltas[3] - deltas[4]);
						if (deltas[3] < deltas[4]) {
							add_score(3, 20.f + (diff > 0.1f ? 5.f : 0.f), 3);
							add_score(4, -5.f, 3);
						} else {
							add_score(4, 20.f + (diff > 0.1f ? 5.f : 0.f), 3);
							add_score(3, -5.f, 3);
						}
					}
					else {
				...
				if (current->m_forward) {
					if (valid_rates[0] && deltas[0] < 0.08f) {
						add_score(0, 12.f, 3);
					} else if (valid_rates[0] && deltas[0] < 0.15f) {
						add_score(0, 8.f, 3);
					} else {
						add_score(0, 5.f, 3);
					}
				...
				if (current->m_processed_velocity && current->m_can_solve_move) {
					const auto prev_side = std::clamp(entry.m_prev_side, 0, 4);
					if (valid_rates[prev_side]) {
						if (deltas[prev_side] < 0.12f) {
							add_score(prev_side, 12.f, 3);
						} else if (deltas[prev_side] < 0.2f) {
							add_score(prev_side, 6.f, 3);
						} else {
							add_score(prev_side, 3.f, 3);
						}
					} else {
						add_score(prev_side, 2.f, 3);
					}
				}
			}
		}

		if (entry.m_trace_side && entry.m_trace_side >= 1 && entry.m_trace_side <= 2 && entry.m_try_trace_resolver) {
			add_score(entry.m_trace_side, 12.f, 2);
		}

		int best_side = current->m_side;
		float best_score = -std::numeric_limits<float>::infinity();
		int best_side_by_history = -1;
		float best_history_score = -std::numeric_limits<float>::infinity();

		for (int side = 0; side < 5; ++side) {
			const auto total_score = scores[side];
			...
			const auto total_attempts = entry.m_side_hits[side] + entry.m_side_misses[side];
			if (total_attempts > 0) {
				const auto hit_ratio = static_cast<float>(entry.m_side_hits[side]) / static_cast<float>(total_attempts);
				const auto history_score = static_cast<float>(entry.m_side_hits[side]) * (7.f + hit_ratio * 4.f)
					- static_cast<float>(entry.m_side_misses[side]) * (7.f + (1.f - hit_ratio) * 3.f);
				
				if (history_score > best_history_score && entry.m_side_hits[side] > 0) {
					best_history_score = history_score;
					best_side_by_history = side;
				}
			}
		}

		if (!std::isfinite(best_score) || best_score < -25.f) {
			if (best_side_by_history >= 0 && entry.m_side_hits[best_side_by_history] >= 2) {
				best_side = best_side_by_history;
			} else {
				best_side = std::clamp(entry.m_prev_side, 0, 4);
			}
		}
		else if (best_side_by_history >= 0 && entry.m_side_hits[best_side_by_history] >= 2) {
			const auto history_bonus = static_cast<float>(entry.m_side_hits[best_side_by_history]) * 6.f;
			const auto history_ratio = static_cast<float>(entry.m_side_hits[best_side_by_history]) 
				/ static_cast<float>(entry.m_side_hits[best_side_by_history] + entry.m_side_misses[best_side_by_history] + 1);
			
			if (history_ratio > 0.55f && scores[best_side_by_history] + history_bonus > best_score * 0.75f) {
				best_side = best_side_by_history;
			}
		}
		if (entry.m_trace_side && entry.m_try_trace_resolver && best_score < 8.f) {
			best_side = entry.m_trace_side;
		}

		current->m_side = best_side;
		entry.m_prev_side = best_side;

		int reason = reasons[best_side];
		switch (reason) {
		case 1:
			current->m_priority = 1;
			current->m_type = 8;

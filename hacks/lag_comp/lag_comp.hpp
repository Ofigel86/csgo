...
namespace supremacy::hacks {
	...
	struct player_entry_t {		
		...
		__forceinline void register_resolve_hit(int side);
		__forceinline void register_resolve_miss(int side);
		__forceinline void decay_history();
		...
		std::array< int, 5u >							m_side_hits{}, m_side_misses{};
		float											m_last_side_update{};

...
namespace supremacy::hacks {
	...
	__forceinline void player_entry_t::reset() {
		...
		m_side_hits.fill(0);
		m_side_misses.fill(0);
		m_last_side_update = 0.f;

		m_lag_records.clear();
	}

	__forceinline void player_entry_t::register_resolve_hit(int side) {
		if (side < 0 || side >= static_cast<int>(m_side_hits.size()))
			return;

		m_side_hits.at(side) = std::min(m_side_hits.at(side) + 1, 20);
		if (m_side_misses.at(side) > 0)
			--m_side_misses.at(side);
		m_last_side_update = valve::g_global_vars->m_real_time;
	}

	__forceinline void player_entry_t::register_resolve_miss(int side) {
		if (side < 0 || side >= static_cast<int>(m_side_misses.size()))
			return;

		m_side_misses.at(side) = std::min(m_side_misses.at(side) + 1, 20);
		m_last_side_update = valve::g_global_vars->m_real_time;
	}

	__forceinline void player_entry_t::decay_history() {
		if (m_last_side_update <= 0.f)
			return;

		if ((valve::g_global_vars->m_real_time - m_last_side_update) < 2.5f)
			return;

		for (auto& value : m_side_hits)
			value = std::max(0, value - 1);

		for (auto& value : m_side_misses)
			value = std::max(0, value - 1);
	...
	__forceinline float c_lag_comp::calc_time_delta(const float sim_time) const {
		...
		float lerp = net_info.m_lerp;
		float latency_in = net_info.m_latency.m_in;
		float latency_out = net_info.m_latency.m_out;
		
		const auto correct = std::clamp(
			lerp + latency_in + latency_out,
		...
		const auto delta = fabs(correct - (valve::to_time(tick_base) - sim_time));
